...
//     const [loadingDrilldown, setLoadingDrilldown] = useState(false);
//     const [selectedRow, setSelectedRow] = useState(null); // { duty_id, team_id }
//     const [drilldownMeta, setDrilldownMeta] = useState({ teamName: '', locationName: '' }); // for export filename
...
//             // Auto column widths based on longest cell value per column
...
//             { label: 'Captain', key: 'team_captain_name' },
//             { label: 'Captain Mobile', key: 'team_captain_mobile' },
...
//         exportToExcel(rows, columns, `Duties_Allotted_${miqaatLabel}`);
//     };
...
//         const safeName = `${drilldownMeta.teamName}_${drilldownMeta.locationName}`.replace(/[^a-zA-Z0-9 _-]/g, '_');
//         exportToExcel(rows, columns, `Guards_${safeName}`);
//     };
...
//                         title="Export guard list to Excel"
//                     >
//                         <i className="ri-file-excel-2-line me-2"></i>
//                         Export to Excel
//                     </Button>
...
const DutiesAllottedReport = ({ moduleId, jamiaatId, title }) => {
    ...
    // ── Main report data ──────────────────────────────────────────────────────
    const [tableData, setTableData] = useState([]);
    const [totalAssignedDirect, setTotalAssignedDirect] = useState(null); // direct count from API, bypasses per-row agg
    const [loading, setLoading] = useState(false);

    // ── Drilldown modal ───────────────────────────────────────────────────────
    // drilldownMode: 'duty' = normal row  (ITS, Name, Position, Mobile, WhatsApp, Age)
    //                'all'  = total row   (ITS, Name, Position, Team, Location, Mobile, Incharge)
    const [showModal, setShowModal] = useState(false);
    const [modalTitle, setModalTitle] = useState('');
    const [drilldownData, setDrilldownData] = useState([]);
    const [loadingDrilldown, setLoadingDrilldown] = useState(false);
    const [drilldownMode, setDrilldownMode] = useState('duty');
    const [drilldownMeta, setDrilldownMeta] = useState({ teamName: '', locationName: '' });
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            if (response.ok && result.success) {
                setTableData(result.data || []);
                setTotalAssignedDirect(result.total_assigned ?? null);
                if ((result.data || []).length === 0) {
                    toast.info('No duty records found for the selected criteria');
                } else {
                    toast.success(`Found ${result.data.length} duty record(s)`);
    ...
    // ── Fetch drilldown — normal duty row ─────────────────────────────────────
    const fetchDrilldown = useCallback(async (row) => {
        setDrilldownData([]);
        setDrilldownMode('duty');
        setLoadingDrilldown(true);
        setShowModal(true);
        setModalTitle(`Guards — ${row.team_name} @ ${row.location_name || 'N/A'}`);
        setDrilldownMeta({ teamName: row.team_name || '', locationName: row.location_name || 'N/A' });
        ...
        try {
            ...
            if (response.ok && result.success) {
                setDrilldownData(result.data || []);
                if ((result.data || []).length === 0) toast.info('No guards assigned to this duty yet');
            } else {
                toast.error(result.message || 'Failed to load guard details');
            }
        } catch (error) {
    ...
    // ── Fetch all guards — total row ──────────────────────────────────────────
    const fetchAllGuards = useCallback(async () => {
        setDrilldownData([]);
        setDrilldownMode('all');
        setLoadingDrilldown(true);
        setShowModal(true);
        setModalTitle(`All Assigned Guards — ${selectedMiqaat?.label || ''}`);
        ...
        try {
            ...
            if (response.ok && result.success) {
                setDrilldownData(result.data || []);
                if ((result.data || []).length === 0) toast.info('No guards assigned for this miqaat yet');
            } else {
                toast.error(result.message || 'Failed to load guard list');
            }
        } catch (error) {
            console.error(error);
            toast.error('Error loading guard list');
        } finally {
            setLoadingDrilldown(false);
        }
    }, [selectedMiqaat, jamiaatId]);
    ...
    const handleExportDrilldown = () => {
        ...
        if (drilldownMode === 'duty') {
            ...
            const rows = drilldownData.map((row, i) => ({ ...row, _sr: i + 1 }));
            exportToExcel(rows, columns, `All_Guards_${miqaatLabel}`);
        }
    };

    // ── Summary card values ───────────────────────────────────────────────────
    const summary = useMemo(() => {
        const totalQuota = tableData.reduce((s, r) => s + (r.quota || 0), 0);
        // Use the direct server-side count when available — guards whose duty_id
        // has no matching row in bg.duties are excluded from per-row sums but
        // are correctly counted in the server-side total_assigned subquery.
        const totalAssigned = totalAssignedDirect ?? tableData.reduce((s, r) => s + (r.assigned_count || 0), 0);
        return {
            totalDuties: tableData.length,
            totalQuota,
            totalAssigned,
            totalBalance: totalQuota - totalAssigned,
        };
    }, [tableData, totalAssignedDirect]);
    ...
    return (
        <Fragment>
                            ...
                            <Card.Body>
                                ...
                                {loading ? (
                                    ...
                                    <div className="table-responsive">
                                        <table className="table table-bordered table-hover">
                                            <thead className="table-light">
                                                <tr>
                                                    ...
                                                    <th>Captain</th>
                                                    <th>Captain Mobile</th>
                                                    <th style={{ width: '70px', textAlign: 'center' }}>Quota</th>
                                                    <th style={{ width: '90px', textAlign: 'center' }}>Assigned</th>
                                                    <th style={{ width: '80px', textAlign: 'center' }}>Balance</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                ...
                                                {tableData.map((row, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td>{row.location_name || '-'}</td>
                                                        <td
                                                            className="incharge-cell"
                                                            title={row.incharge_name || ''}
                                                        >
                                                            {row.incharge_name || '-'}
                                                        </td>
                                                        <td>{row.team_name || '-'}</td>
                                                ...
                                                {/* ── Total row — click opens all-guards modal ── */}
                                                <tr
                                                    className="total-row"
                                                    onClick={fetchAllGuards}
                                                    title="Click to view all assigned guards for this miqaat"
                                                >
                                                    <td colSpan={7} style={{ textAlign: 'right', paddingRight: '16px' }}>
                                                        <i className="ri-bar-chart-grouped-line me-2" style={{ opacity: 0.7 }}></i>
                                                        TOTAL
                                                    </td>
                                                    <td style={{ textAlign: 'center' }}>{summary.totalQuota}</td>
                                                    <td style={{ textAlign: 'center' }}>{summary.totalAssigned}</td>
                                                    <td style={{ textAlign: 'center' }}>{summary.totalBalance}</td>
                                                </tr>
                                        ...
                                        <small className="text-muted">
                                            <i className="ri-mouse-line me-1"></i>
                                            Click any row to view assigned guards &nbsp;·&nbsp;
                                            Click <strong>TOTAL</strong> to view all guards for this miqaat
                                        </small>
                                    </div>
                                )}
                            </Card.Body>
            ...
            {/* ════════════════════════════════════════════════════════════════
                Drilldown Modal
                — 'duty' mode : normal row columns (name, position, mobile…)
                — 'all'  mode : full guard details (team, location, incharge…)
            ════════════════════════════════════════════════════════════════ */}
            <Modal
                show={showModal}
                onHide={() => setShowModal(false)}
            ...
            >
                ...
                <Modal.Body style={{ padding: '20px' }}>
                    {loadingDrilldown ? (
                        ...
                        <div className="text-center py-4">
                            <i className="ri-user-unfollow-line" style={{ fontSize: '48px', color: '#dee2e6' }}></i>
                            <p className="mt-2 text-muted">No guards assigned yet</p>
                        </div>
                    ) : (
                        <>
                            <div className="mb-3">
                                        ...
                                        <tbody>
                                            {drilldownData.map((guard, index) => (
                                                <tr key={guard.guard_duty_id || guard.its_id}>
                                                    ...
                                                    <td>{guard.mobile || '-'}</td>
                                                    <td style={{ fontSize: '12px', color: '#6c757d', fontStyle: 'italic' }}>
                                                        {guard.incharge_name || '-'}
                                                    </td>
                                                </tr>
                                            ))}
