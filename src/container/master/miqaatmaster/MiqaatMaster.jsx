...
// //     // NEW: Enhanced success alert with multiple navigation options
...
// //             `,
// //             icon: 'success',
// //             showDenyButton: !isClone, // Only show "Add Duties" if NOT cloning
// //             showCancelButton: true,
// //             confirmButtonColor: '#0d6efd',
// //             denyButtonColor: '#28a745',
// //             cancelButtonColor: '#6c757d',
// //             confirmButtonText: `<i class="ri-user-add-line me-2"></i>${isClone ? 'Show Incharges' : 'Add Incharges'}`,
// //             denyButtonText: '<i class="ri-shield-user-line me-2"></i>Add Duties',
...
// //                 // Show navigation dialog with multiple options
...
//             `,
//             icon: 'success',
//             showDenyButton: true, // Always show both buttons
//             showCancelButton: true,
//             confirmButtonColor: '#0d6efd',
//             denyButtonColor: '#28a745',
//             cancelButtonColor: '#6c757d',
...
//                 // Navigate to Duties page
//                 handleClose();
//                 navigate(`${import.meta.env.BASE_URL}backoffice/dutiesassign`, {
//                     state: {
//                         miqaatId: newMiqaatId,
//                         miqaatName: newMiqaatName,
//                         venueId: newVenueId,
...
//         // NEW: Validate Reporting Time is before Start Time
//         if (formData.startDate && formData.startTime && formData.reportingTime) {
//             const startDateTime = new Date(`${formData.startDate}T${formData.startTime}`);
//             const reportingDateTime = new Date(`${formData.startDate}T${formData.reportingTime}`);
//             if (reportingDateTime >= startDateTime) {
//                 newErrors.reportingTime = 'Reporting Time must be before Miqaat Start Time';
//             }
//         }
...
//                         <div className="datetime-row">
//                             <div className="date-input"><Form.Control ref={startDateRef} type="date" name="startDate" value={formData.startDate} onChange={handleInputChange} onClick={() => openPicker(startDateRef)} className={errors.startDate ? 'is-invalid' : ''} disabled={loading} />{errors.startDate && <div className="error-text">{errors.startDate}</div>}</div>
//                             <div className="time-input"><Form.Control ref={startTimeRef} type="time" name="startTime" value={formData.startTime} onChange={handleInputChange} onClick={() => openPicker(startTimeRef)} className={errors.startTime ? 'is-invalid' : ''} disabled={loading} step="900" />{errors.startTime && <div className="error-text">{errors.startTime}</div>}</div>
//                         </div>
//                     </div>
//                 </div>
//                 <div className="horizontal-form-group">
...
//                         <div className="datetime-row">
//                             <div className="date-input"><Form.Control ref={endDateRef} type="date" name="endDate" value={formData.endDate} onChange={handleInputChange} onClick={() => openPicker(endDateRef)} className={errors.endDate ? 'is-invalid' : ''} disabled={loading} />{errors.endDate && <div className="error-text">{errors.endDate}</div>}</div>
//                             <div className="time-input"><Form.Control ref={endTimeRef} type="time" name="endTime" value={formData.endTime} onChange={handleInputChange} onClick={() => openPicker(endTimeRef)} className={errors.endTime ? 'is-invalid' : ''} disabled={loading} step="900" />{errors.endTime && <div className="error-text">{errors.endTime}</div>}</div>
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//                     <Form.Label className="form-label">Reporting Time <span className="text-danger">*</span></Form.Label>
//                     <div className="form-input-wrapper">
//                         <Form.Control ref={reportingTimeRef} type="time" name="reportingTime" value={formData.reportingTime} onChange={handleInputChange} onClick={() => openPicker(reportingTimeRef)} className={errors.reportingTime ? 'is-invalid' : ''} disabled={loading} step="900" />
//                         {errors.reportingTime && <div className="error-text">{errors.reportingTime}</div>}
//                     </div>
//                 </div>
//                 <div className="horizontal-form-group">
...
const TimeSplitInput = ({ value, onChange, disabled, error }) => {
    ...
    const parseTime = (val) => {
        ...
        try {
            // Handle "HH:mm:ss" coming from backend if any
    ...
    const handleChange = (field, val) => {
        ...
        if (newState.hour && newState.minute && newState.period) {
            ...
            // value exists but current selection incomplete? 
            // It's better to only trigger onChange when valid. 
            // Or trigger empty if cleared?
            // For now, only trigger when full.
        }
    };
                ...
                <div className="horizontal-form-group">
                    ...
                    <div className="form-input-wrapper">
                        <TimeSplitInput
                            ...
                            onChange={(val) => handleTimeChange('reportingTime', { value: val })}
                            disabled={loading}
...
const EditMiqaat = ({
    ...
    const validateForm = () => {
        ...
        // Validate Reporting Time <= Start Time
        if (formData.startDate && formData.startTime && formData.reportingTime) {
            const startDateTime = new Date(`${formData.startDate}T${formData.startTime}`);
            const reportingDateTime = new Date(`${formData.startDate}T${formData.reportingTime}`);
            if (reportingDateTime > startDateTime) {
                newErrors.reportingTime = 'Reporting Time must be before or equal to Miqaat Start Time';
            }
        }
