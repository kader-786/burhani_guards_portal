...
const AddVenue = ({
    ...
    const handleAddLocation = () => {
        const locationName = formData.locationName.trim();
        const locationColor = formData.locationColor.toUpperCase();
        ...
        >
            <style>
                {`
                    ...
                    .location-input-group .color-picker-wrapper {
                        width: 50px;
                        flex-shrink: 0;
                    }

                    .location-input-group .color-picker-wrapper input[type="color"] {
                        width: 50px;
                        height: 38px;
                        border: 1px solid #dee2e6;
                        border-radius: 6px;
                    ...
                    .location-input-group .color-picker-wrapper input[type="color"]::-webkit-color-swatch-wrapper {
                        padding: 0;
                    }

                    .location-input-group .color-picker-wrapper input[type="color"]::-webkit-color-swatch {
                        border: none;
                        border-radius: 4px;
                    }
            ...
            {/* Row 3: Duty Location with Color Picker */}
            <div className="horizontal-form-group">
                <Form.Label className="form-label">Duty Location</Form.Label>
                <div className="form-input-wrapper">
                    <div className="location-input-group">
                            ...
                            <tbody>
                                {locations.map((loc, index) => (
                                    <tr key={loc.id}>
                                        <td>{index + 1}</td>
                                        <td>
                                            <div className="color-badge" style={{ backgroundColor: loc.location_color }}></div>
                                        </td>
                                        <td>{loc.location_name}</td>
                                        <td>
                                            <Button
...
const EditVenue = ({
        ...
        try {
            ...
            if (response.ok && result.success && result.data && result.data.length > 0) {
                ...
                const loadedLocations = (venueData.locations || []).map((loc, index) => ({
                    ...
                    location_id: loc.location_id,
                    location_name: loc.location_name,
                    location_color: loc.location_color || '#808080',
                    flag: 'X'
                }));

                setLocations(loadedLocations);
                setOriginalLocations(JSON.parse(JSON.stringify(loadedLocations)));
                setLocationCounter(loadedLocations.length + 1);
            } else {
                Swal.fire({
                    icon: 'error',
    ...
    const handleColorChange = (id, newColor) => {
        setLocations(prev => prev.map(loc => {
            if (loc.id === id && loc.flag === 'X') {
                return { ...loc, location_color: newColor.toUpperCase() };
            }
            return loc;
        }));
    };
    ...
    const hasChanges = () => {
        ...
        const locationsChanged = locations.some(loc => {
            if (loc.flag === 'I' || loc.flag === 'D') return true;

            const original = originalLocations.find(o => o.id === loc.id);
            if (!original) return true;

            return loc.location_color !== original.location_color ||
                loc.location_name !== original.location_name;
        });
    ...
    const handleReset = () => {
        if (originalData) {
            setFormData({ ...originalData, locationName: '', locationColor: '#808080' });
            setErrors({});
        ...
        try {
            ...
            if (result.success && result.data) {
                const transformedData = result.data.map((item, index) => ({
                    ...
                    srNo: index + 1,
                    venueName: item.venue_name,
                    venueTypeName: item.venue_type_name || 'N/A',
                    jamiaatName: item.jamiaat_name || 'N/A'
                }));
                setTableData(transformedData);
            } else {
                throw new Error(result.message || 'Failed to fetch venues');
